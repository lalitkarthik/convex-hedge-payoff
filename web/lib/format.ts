 ...
 *    worst kind of wrong. The fixture stems are UTC; nothing renders one raw.
...
export function istClock(stem: string): string {
  const utc = new Date(`${stem.split("T")[0]}T${stem.split("T")[1].replaceAll("-", ":")}Z`);
  const ist = new Date(utc.getTime() + IST_OFFSET_MINUTES * 60_000);
  return `${String(ist.getUTCHours()).padStart(2, "0")}:${String(ist.getUTCMinutes()).padStart(2, "0")}`;
}

/** The full IST stamp, for the header. */
export function istStamp(stem: string): string {
  return `${istClock(stem)} IST`;
}
